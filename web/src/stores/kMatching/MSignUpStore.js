...
export default class SignUpStore {
    ...
    doChangePassword = flow(function* doChangePassword() {
        ...
        try {
            const param = toJS(this.newMember);
            delete param.passwordConfirm;
            const resp = yield axios.post('/api/v1/kMatching/users/changepassword', param)
        } catch (e) {
            console.log(e)
            this.state = State.Fail;
        }finally {
    ...
    // 아이디찾기 닉네임, 아이디 체크 20.10.30[이지현]
    doCheckName = flow(function* doChackName() {
        this.state = State.Pending;
        try {
            const param = toJS(this.newMember);
            delete param.passwordConfirm;
            const resp = yield axios.post('/api/v1/kMatching/users/nameck', param)
                .then (res => {
                    if(res.data.result){
