...
@RestController
@RequestMapping("/api/v1/kTagging/authentications/")
public class TaggingAuthenticationController {
    private TaggingAuthenticationService authenticationService;

    @Autowired
    public TaggingAuthenticationController(TaggingAuthenticationService authenticationService) {
    ...
    @PostMapping("/signin")
    public ResponseEntity<TaggingUserToken> getLoginTokenTagging(HttpServletRequest httpRequest, HttpSession session, @RequestBody TaggingUser account) {
        final TaggingUserToken token = authenticationService.getToken(account.getId(), account.getPassword(), session);
    ...
    public ResponseEntity<TaggingSimpleUser> checkTagging(HttpServletRequest httpRequest) {
        final TaggingSimpleUser user = authenticationService.getUser();
