...
@RestController
@RequestMapping("/api/v1/kfashion/stylejson")
public class JsonByStyleController {
    private static final Logger logger = LoggerFactory.getLogger(JsonByStyleController.class);
    ...
    @Autowired
    public JsonByStyleController(KfashionLabelService kfashionLabelService,
...
//        String[] styleList = kfashionWorkService.selectJsonStyleList();
//        logger.debug(" styleList {} ", styleList.length);
//        logger.debug(" workNo {} ", workNo.length);
//
//        if(styleList.length > 0) {
//            for (String value : styleList){
...
//
//
//                        jsonObject.put("파일 번호", workNo[z]);
//                        String workName = kfashionWorkService.selectWorkName(workNo[z]);
//                        String workDataTime = kfashionWorkService.selectWorkDataTime(workNo[z]);
//
//                        jsonObject.put("파일 이름", workName);
//                        jsonObject.put("파일 생성일자", workDataTime);
//
...
//                        descriptionObject.put("라벨링", labelObject);
//
...
//
//                        descriptionObject.put("렉트좌표", rectObject);
//
//
...
//
//                        descriptionObject.put("폴리곤좌표", polygonObject);
//
//
//                        jsonObject.put("데이터셋 상세설명", descriptionObject);
//
//                        logger.debug("styleMap.get() {} ", styleMap.get("스타일"));
////                        List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
////                        if( style != null) {
////                            int jasonSize = style.size();
////                            for(int x = 0; x < jasonSize; x++) {
////                                Map<String, Object> map = JsonUtil.getMapFromJsonObject(
////                                        (JSONObject) style.get(x)
////                                );
////                            }
////                        }
//                        if(value.equals(styleMap.get("스타일"))) {
//                            logger.debug(" value {} ", value);
//                            logger.debug(" String.valueOf(style) {} ", String.valueOf(style));
//
//                            String stylePath = "/Users/kwon/Documents/project/kfashion/json/" + value; //폴더 경로
//
...
//                            }
//
//                            String path = "/Users/kwon/Documents/project/kfashion/json/" + value + "/" + workNo[z]; //폴더 경로
//
...
//                            imageObject.put("이미지 높이", image.getHeight(null));
//                            imageObject.put("이미지 너비", image.getWidth(null));
//
//                            allObject.put("이미지 정보", imageObject);
//                            allObject.put("데이터셋 정보", jsonObject);
//
//
//                            FileWriter file = new FileWriter("/Users/kwon/Documents/project/kfashion/json/" + value + "/" + workNo[z] + "/" + workNo[z] + ".json");
...
//    }
    ...
    public ResponseEntity<Object> jsonFileUp(HttpServletRequest httpRequest) throws IOException {
        ...
        String defaultZipPath = "/Users/kwon/Documents/project/kfashion/json/";
        ArrayList<String> outputStyleList = new ArrayList<>();
        Long[] workNo = kfashionWorkService.selectJsonWorkList();
        //String[] styleList = kfashionWorkService.selectJsonStyleList();
        //logger.debug(" styleList {} ", styleList.length);
        logger.debug(" workNo {} ", workNo.length);
...
//                            }
                        ...
                        String path = "/Users/kwon/Documents/project/kfashion/json/" + styleMap.get("스타일") + "/" + workNo[z]; //폴더 경로

                        File StyleFolder = new File(path);
                        ...
                        FileWriter file = new FileWriter("/Users/kwon/Documents/project/kfashion/json/" + styleMap.get("스타일") + "/" + workNo[z] + "/" + workNo[z] + ".json");
                        String styleName = styleMap.get("스타일").toString();

                        file.write(allObject.toJSONString());
                        file.flush();
                        file.close();
                        //contains를 사용한 List 중복제거
                        if(!outputStyleList.contains(styleName)) {
                            outputStyleList.add(styleName);
                        }
                        fileNum++;
                        logger.debug("fileNum {}", fileNum);

//                        //파일압축
//                       CompessZip compessZip = new CompessZip();
//
//                        String zipPath = "/Users/kwon/Documents/project/kfashion/json/" + styleMap.get("스타일") + "/";
//                        String outputZipPath = "/Users/kwon/Documents/project/kfashion/json/";
//                        String zipFileName = styleMap.get("스타일").toString();
//
//                        //압축하기
//                        try {
//                            if(!compessZip.compress(zipPath, outputZipPath, zipFileName)) {
//                                logger.debug("압축실패");
//                                return new ResponseEntity<Object>(fileNum + " 파일 압축 실패", HttpStatus.OK);
//                            }
//                        }catch (Throwable e) {
//                            e.printStackTrace();
//                        }
                ...
                logger.debug("outputStyle {}", outputStyleList);
        //파일압축
        CompessZip compessZip = new CompessZip();
        for(String value : outputStyleList) {
            String zipPath = defaultZipPath + value + "/";
            String outputZipPath = defaultZipPath;
            String zipFileName = value;
        ...
        logger.debug(" JSon Method Start Time {}", start);
        logger.debug(" JSon Method end Time {}", end);
        logger.debug(" JSon Method Total Time/sec {}", (end-start)/1000);
        logger.debug(" JSon Method Start startTime {}", startTime);
        logger.debug(" JSon Method endTime {}", endTime);
        return new ResponseEntity<Object>(fileNum + " 파일 압축 성공", HttpStatus.OK);
    }
